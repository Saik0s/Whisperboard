...
final class LocalTranscriptionWorkExecutor: TranscriptionWorkExecutor {
  ...
  fileprivate func whisperCPPProcess(_ task: TranscriptionTaskEnvelope) async throws {
  ...
  fileprivate func whisperKitProcess(_ task: TranscriptionTaskEnvelope) async throws {
  ...
  func process(task: TranscriptionTaskEnvelope) async {
    ...
    do {
      ...
      try await whisperCPPProcess(task)
    } catch {
      await MainActor.run {
        task.recording.transcription?.status = .error(message: error.localizedDescription)
      }
