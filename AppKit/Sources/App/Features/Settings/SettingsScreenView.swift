  ...
  var body: some View {
    NavigationStack {
      Form {
        ...
        #if DEBUG
          SubscriptionSectionView(viewStore: viewStore)
    ...
    .alert(store: store.scope(state: \.$alert, action: { .alert($0) })).task { viewStore.send(.task) }.onAppear {
  ...
  var body: some View {
    Section {
      SettingsSheetButton(
        ...
        iconBGColor: .DS.Background.accent,
